...
def authenticate():
    ...
    client_id = "11754030-fc21-4204-88cd-89e8c2a48f9a_2ab1ea6e-b39d-448e-8ea2-ddcd14c4150b"
    client_secret = "JQE05BAsAzLczi/OenKzgH9QIzX50KmNa10iSbZLaDc="
    scope = "icdapi_access"
...
def parse_and_store_tree(
    root_uri, output_file, token=authenticate(), max_workers=4, checkpoint_interval=10
):
    unified_json = {}
    processed_nodes = set()
    ...
    def recursive_parse(uri):
        ...
        parents = (
            [pt.split("/")[-1] for pt in item["parent"]]
            if item["parent"] != "Key Not found"
            else []
        )

        children = (
            [ct.split("/")[-1] for ct in item["child"]] if item["child"] != "Key Not found" else []
        )

        definition = item["definition"]["@value"] if item["definition"] != "Key Not found" else None

        synonyms = (
            [syn["label"]["@value"] for syn in item["synonym"]]
            if item["synonym"] != "Key Not found"
            else []
        )
